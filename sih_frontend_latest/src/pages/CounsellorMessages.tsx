...
import { matchesCaseSearch } from "../lib/caseIdentity";
...
export default function CounsellorMessages() {
  ...
  return <div className="space-y-5"><div><h1 className="text-2xl font-bold">Messages</h1><p className="mt-1 text-sm text-muted-foreground">Conversations with your assigned cases. Messages persist across sessions.</p></div>
    ...
    <div className="grid items-start gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      <Card><CardContent className="space-y-2 pt-5"><Input aria-label="Search conversations by victim name or case ID" placeholder="Search name or case ID..." value={search} onChange={e => setSearch(e.target.value)} />
      ...
      {conversations.filter(c => matchesCaseSearch(c, search)).map(c => <button key={c.caseId} onClick={() => selectConversation(c.caseId)} className={`w-full rounded-lg border p-3 text-left text-sm ${selectedConversation?.caseId === c.caseId ? "border-primary bg-primary/5 text-primary" : "border-border"}`}><MessageCircle className="mr-2 inline h-4 w-4" /><CaseIdentity caseId={c.caseId} victimName={c.victimName} /></button>)}</CardContent></Card>
