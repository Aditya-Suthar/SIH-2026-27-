  ...
  return <div className="space-y-4">
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-3"><div><p className="text-xs text-muted-foreground">Latest successful distress indicator</p><p className="mt-1 text-3xl font-semibold">{data.latest_analysis?.distress_score ?? "—"}<span className="text-sm font-normal text-muted-foreground">{data.latest_analysis ? " / 100" : " No result yet"}</span></p></div><div><p className="text-xs text-muted-foreground">Risk indicator</p><Badge className="mt-2" variant={riskTone(data.latest_analysis?.risk_level)}>{data.latest_analysis?.risk_level ?? "Unknown"}</Badge></div><div><p className="text-xs text-muted-foreground">Historical trend</p><p className="mt-2 text-base font-medium capitalize">{words(data.trend.state)}</p></div></div>
