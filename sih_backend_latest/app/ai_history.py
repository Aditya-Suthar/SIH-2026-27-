...
class AnalysisOut(BaseModel):
    ...
    source_type: Literal['assessment', 'message'] = 'assessment'
    case_id: int
    victim_id: int
...
def case_analysis_history(
    ...
    analyses = valid_source_query(db).filter(models.AIAnalysis.case_id == case.id)
    if user.role == 'victim':
...
def valid_source_query(db):
    """Keep both source types scoped to their real case, not just a supplied ID."""
    from sqlalchemy import and_, or_
    return db.query(models.AIAnalysis).outerjoin(
        models.Assessment, models.Assessment.id == models.AIAnalysis.assessment_id,
    ).outerjoin(
        models.CaseMessage, models.CaseMessage.id == models.AIAnalysis.message_id,
    ).filter(or_(
        and_(models.Assessment.id.is_not(None), models.Assessment.case_id == models.AIAnalysis.case_id),
        and_(models.CaseMessage.id.is_not(None), models.CaseMessage.case_id == models.AIAnalysis.case_id,
             models.CaseMessage.victim_id == models.AIAnalysis.victim_id,
             models.CaseMessage.sender_role == 'victim'),
    ))
