...
const transcribeRecording = async (
  ...
  try {
    ...
    //
    // Whisper transcript becomes the SAME `note`
    // used by the existing AI assessment workflow.
    // ================================================
    ...
    alert(
      error instanceof Error
        ? error.message
        : "Could not transcribe voice recording"
    );
  ...
  prev
    ...
    const analysisNotice = result.ai_analysis?.status === "failed"
      ...
      : result.ai_analysis?.status === "pending"
        ...
        : "";
    alert(
      `Assessment submitted. Distress Score: ${result.distressScore}, Risk: ${result.riskLevel}${analysisNotice}`
    );
  } catch (error) {
    console.error(error);
